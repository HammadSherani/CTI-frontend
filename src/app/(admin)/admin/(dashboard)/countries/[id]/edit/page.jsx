...
function EditCountry() {
  ...
  const fetchCountry = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get(`/admin/countries/${params.id}`, {
        headers: {
      ...
      setValue('latitude', country.coordinates?.latitude || '');
      setValue('longitude', country.coordinates?.longitude || '');
    } catch (error) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await axiosInstance.put(`/admin/countries/${params.id}`, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setSubmitSuccess('Country updated successfully!');
      ...
      const errorMessage =
        error.response?.data?.message || error.message || 'Failed to update country. Please try again.';
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500 flex items-center">
                ...
                Start typing to see country suggestions. Coordinates will auto-fill.
              </p>
              {errors.name && (
                ...
                <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-800">
                        {watchedLatitude && `Latitude: ${watchedLatitude}°`}
                        {watchedLatitude && watchedLongitude && ' | '}
                        {watchedLongitude && `Longitude: ${watchedLongitude}°`}
                      </p>
                    </div>
                  </div>
                </div>
