...
/* ══════════════════════════════════════════════════════════
   AVATAR
══════════════════════════════════════════════════════════ */
function Avatar({ name, size = 38, isAdmin = false }) {
...
function Bubble({ msg }) {
  ...
  return (
    <div className={`flex gap-3 mb-4 ${isAdmin ? 'flex-row-reverse' : 'flex-row'}`}>
      <Avatar
        name={msg.senderName || (isAdmin ? 'Admin' : 'Customer')}
        size={36}
        isAdmin={isAdmin}
      />

      <div className={`max-w-[75%] sm:max-w-[60%] flex flex-col ${isAdmin ? 'items-end' : 'items-start'}`}>
        <div className="flex items-center gap-2 mb-1 px-1">
          <span className="text-xs font-semibold text-gray-700">{msg.senderName || (isAdmin ? 'You' : 'Customer')}</span>
...
export default function EnquiryDetailPage() {
  ...
  const fetchQuery = useCallback(async () => {
    if (!token || !id) return;
    try {
      const { data } = await axiosInstance.get(`/admin/refurbish/queries/${id}`, { headers });
      if (data.success) {
        setQuery(data.data);
      }
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Failed to load enquiry');
      router.push('/admin/refurbished/enquiries');
    } finally {
      setLoading(false);
    }
  }, [id, token, router]);
  ...
  const handleReply = async () => {
    ...
    try {
      const { data } = await axiosInstance.post(
        `/admin/refurbish/queries/${id}/reply`,
  ...
  const handleStatus = async (status) => {
    ...
    try {
      const { data } = await axiosInstance.patch(
        `/admin/refurbish/queries/${id}/status`,
        { status },
        { headers }
      );
      if (data.success) {
    ...
    return (
      <div className="h-[calc(100vh-100px)] bg-gray-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <Icon icon="svg-spinners:180-ring-with-bg" className="w-10 h-10 text-amber-500" />
  ...
  return (
    <div className="h-[calc(100vh-100px)] mb-10 flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="flex-shrink-0 bg-white border-b border-gray-200 z-20">
        <div className="flex items-center gap-4 px-6 py-4 max-w-7xl mx-auto w-full">
          {/* Back */}
          <button
            onClick={() => router.push('/admin/refurbished/enquiries')}
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              {!isClosed ? (
                ...
                <button
                  ...
                  disabled={statusBusy}
                  className="flex items-center gap-1.5 bg-amber-500 hover:bg-amber-600 text-white font-medium px-4 py-2 rounded-lg text-sm transition-colors"
                >
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden max-w-7xl mx-auto w-full">
        ...
        <div className="hidden lg:flex flex-col w-72 flex-shrink-0 bg-white border-r border-gray-200 overflow-y-auto py-6 px-6">
          ...
          <div className="mb-6">
            ...
            <InfoRow label="Email" value={query.customerEmail || '—'} />
          </div>
          ...
          {!isOrderQ && query.productId && (
            <div className="mb-6">
              ...
              {query.productId?.title && <InfoRow label="Product Name" value={query.productId.title} />}
            </div>
          )}
            ...
            {grouped.length === 0 ? (
              ...
              grouped.map((item, i) =>
                item.type === 'date'
                  ...
                  : <Bubble key={item.msg._id || i} msg={item.msg} />
              )
            )}
                ...
                <div className="flex flex-wrap gap-2">
                  {TEMPLATES.map(t => (
                    <button
                      ...
                      className="text-xs bg-gray-50 text-gray-700 border border-gray-200 hover:border-amber-300 hover:text-amber-700 hover:bg-amber-50 font-medium px-3 py-1.5 rounded-lg transition-colors"
                    >
                      {t.label}
                    </button>
                  ))}
            ...
            {isClosed ? (
              ...
              <div className="flex items-end gap-3">
                ...
                <button
                  ...
                  title="Quick replies"
                  className={`flex-shrink-0 p-3 rounded-xl transition-colors border ${showTemplates ? 'bg-amber-50 border-amber-200 text-amber-600' : 'bg-white border-gray-200 hover:bg-gray-50 text-gray-500'
                    }`}
                >
                ...
                {/* Textarea */}
                <div className="flex-1 bg-white border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-amber-500/20 focus-within:border-amber-500 transition-all">
                  <textarea
                    ref={textareaRef}
                    rows={1}
                    value={replyText}
                ...
                <button
                  ...
                  className="flex-shrink-0 px-6 py-3 bg-amber-500 hover:bg-amber-600 disabled:bg-gray-300 disabled:text-gray-500 text-white font-medium rounded-xl transition-all flex items-center justify-center gap-2"
                >
                  {sending ? (
