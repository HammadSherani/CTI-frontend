...
function AdminCategoriesPage() {
    ...
    useEffect(() => {
        fetchCategories();
    }, [currentPage, searchTerm, filterActive, filterFeatured, filterParent]);
    ...
    const fetchCategories = async () => {
        try {
            ...
            const params = new URLSearchParams({
                ...
                ...(filterFeatured !== 'all' && { 
                    isFeatured: filterFeatured === 'featured' ? 'true' : 'false' 
                }),
                ...(filterParent !== 'all' && { 
                    parent: filterParent === 'main' ? 'null' : filterParent 
                })
            });
            ...
            if (response.data.pagination) {
                ...
                // If no pagination, create it from data
                setPagination({
                    currentPage: 1,
                    totalPages: 1,
    ...
    const toggleActive = async (categoryId) => {
        try {
            const res = await axiosInstance.patch(`/admin/categories/${categoryId}/toggle-status`, 
                {},
                {
                    headers: {
                        'Authorization': `Bearer ${token}`
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                                            ...
                                            <tr key={category._id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4">
                                                    {category.parent ? (
                                                        ...
                                                        <span className="text-xs text-gray-400">Main Category</span>
                                                    )}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    ...
                                                    >
                                                        <Icon
                                                            icon={category.isActive ? 'mdi:check-circle' : 'mdi:close-circle'}
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="bg-white p-6 rounded-lg shadow-sm">
                        <div className="flex items-center">
                            <div className="flex-shrink-0">
                                <Icon icon="mdi:folder-multiple" className="w-8 h-8 text-primary-600" />
                            </div>
                            <div className="ml-4">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm">
                        <div className="flex items-center">
                            <div className="flex-shrink-0">
                                <Icon icon="mdi:folder-outline" className="w-8 h-8 text-blue-600" />
                            </div>
                            <div className="ml-4">
                                <p className="text-sm font-medium text-gray-500">Main Categories</p>
                                <p className="text-2xl font-semibold text-gray-900">
                                    {loading ? '...' : categories.filter(cat => !cat.parent).length}
                                </p>
                            </div>
                        </div>
                    </div>
