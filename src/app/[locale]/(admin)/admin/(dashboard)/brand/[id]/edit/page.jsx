...
function EditBrand() {
    ...
    const fetchBrand = async () => {
        try {
            setLoading(true);
            const response = await axiosInstance.get(`/admin/brands/${brandId}`, {
                headers: {
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                setRemoveCurrentIcon(false); // Reset remove flag if new image selected
            };
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const response = await axiosInstance.put(`/admin/brands/${brandId}`, formData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            setSubmitSuccess('Brand updated successfully!');
            
            // Update current icon if changed
            const updatedBrand = response.data.data.brand;
            setCurrentIcon(updatedBrand.icon);
            ...
            // Optional: Navigate back to brand list after 2 seconds
            setTimeout(() => {
            ...
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update brand. Please try again.';
            setSubmitError(errorMessage);
            toast.error(errorMessage);
        }
    };
            ...
            <div className="max-w-4xl mx-auto">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600 mt-1">Update brand information and logo</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-sm p-6">
                    <div className="space-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {currentIcon && !removeCurrentIcon ? 'Update Logo' : 'Brand Logo'}
                            </label>
                            <p className="text-sm text-gray-500 mb-4">
                                Upload a new brand logo image (optional). Supported formats: JPG, PNG, GIF, WebP, SVG. Max size: 2MB.
                            </p>
                            ...
                            <div className="space-y-4">
                                ...
                                {imagePreview && (
                                    <div className="relative inline-block">
                                        ...
                                        <p className="text-sm text-primary-600 mt-2 text-center">New logo preview</p>
                                    </div>
                                )}
                        ...
                        <div className="border-t pt-6">
                            ...
                            <div className="bg-gray-50 rounded-lg p-6">
                                <div className="flex items-center gap-4">
                                    <div className="flex items-center justify-center w-16 h-16 bg-white rounded-lg shadow-sm overflow-hidden">
                                        {imagePreview ? (
                                            <img
                                                src={imagePreview}
                                                alt="Preview"
                                            ...
                                            <Icon icon="mdi:domain" className="w-8 h-8 text-gray-400" />
                                        )}
                                    </div>
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">
                                            {watchedName ? `${watchedName} devices and accessories` : 'Brand description will appear here'}
                                        </p>
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                                ...
                                                <Icon 
                                                    icon={watchedIsActive ? 'mdi:eye' : 'mdi:eye-off'} 
                        ...
                        <div className="flex justify-end gap-3 pt-6 border-t">
                            ...
                            >
                                ...
                                {uploadProgress > 0 ? `Uploading ${uploadProgress}%` : 'Update Brand'}
                            </button>
                        </div>
                    </div>
                </div>
