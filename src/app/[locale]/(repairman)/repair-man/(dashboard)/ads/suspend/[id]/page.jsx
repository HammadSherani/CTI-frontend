...
// Validation schema - FIXED: Removed currency requirement since it's not needed for update
...
function SuspendAdvertisment() {
  const router = useRouter();
  const params = useParams();
  const { id } = params;
  console.log('Ad ID from URL:', id);
  ...
  const fetchAdDetails = async () => {
    try {
      ...
      console.log('Fetched Ad Data:', data.data);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await axiosInstance.patch(`/repairman/advertisements/${id}`, formData, {
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Suspension Reason Display - Only show if suspended */}
        {currentAd.status === 'suspended' && currentAd.reason && (
          <div className="bg-red-50 border-l-4 border-red-500 rounded-lg shadow-sm mb-6 overflow-hidden">
            <div className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-red-900 mb-2">
                    Advertisement Suspended
                  </h3>
                  <div className="bg-white rounded-lg p-4 border border-red-200">
                    <p className="text-sm font-medium text-gray-700 mb-3">
                      <span className="text-red-700 font-semibold">Reason:</span> {currentAd.reason.title || 'No reason provided'}
                    </p>
                    {currentAd.reason.image && (
                      <div className="mt-3">
                        <p className="text-xs text-gray-600 mb-2">Suspension Evidence:</p>
                        <div className="relative w-full max-w-md">
                          <img
                            src={currentAd.reason.image}
                            alt="Suspension reason screenshot"
                  ...
                  <p className="text-sm text-red-700 mt-3">
                    Please review the reason above, make necessary changes to your advertisement, and resubmit for approval.
                  </p>
                </div>
              </div>
            </div>
        ...
        {/* Rejection Info for other rejected statuses */}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg border-2 border-gray-200">
              <div className="flex items-center gap-3">
                <Icon
                  icon={watchType === 'service' ? 'mdi:wrench' : 'mdi:account-circle'}
                  className="text-3xl text-primary-600"
                />
                <div>
                  <p className="font-semibold text-gray-900 capitalize">{watchType} Advertisement</p>
                  <p className="text-sm text-gray-600">Type cannot be changed</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {watchType === 'profile' && currentAd.user_id && (
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Profile Information</h2>
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-gray-600 mb-2">Profile</p>
                <p className="font-semibold text-gray-900">
                  {currentAd.user_id?.repairmanProfile?.fullName || 'N/A'}
                </p>
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  ...
                  {currentAd.status === 'suspended' ? 'Resubmit Advertisement' : 'Update Advertisement'}
                </>
              )}
            </button>
          </div>
...
export default SuspendAdvertisment;