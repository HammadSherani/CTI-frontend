  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axiosInstance.post(
        `/seller/refurbished-products/products/${product._id}/variants`,
        fd,
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-[10px] text-gray-400 flex items-center gap-1">
                <span className="cursor-pointer hover:text-primary-500 transition-colors" onClick={() => router.push("/seller/refurbished/products")}>
                  Products
                </span>
...
────────────────────────────────────────────────────────── */
export default function AdminRefurbishVariantManagementPage({ params }) {
  const { token } = useSelector((s) => s.auth);
    ...
    (async () => {
      try {
        const [prodRes, varRes] = await Promise.all([
          axiosInstance.get(`/seller/refurbished-products/products/${productId}`, { headers: { Authorization: `Bearer ${token}` } }),
          axiosInstance.get(`/seller/refurbished-products/products/${productId}/variants`, { headers: { Authorization: `Bearer ${token}` } }),
        ]);
