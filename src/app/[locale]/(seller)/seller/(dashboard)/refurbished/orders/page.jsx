...
export default function RefurbishedOrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axiosInstance.get("/seller/refurbished-orders", {
  ...
  const handleStatusUpdate = async (orderId, newStatus) => {
    ...
    try {
      const { data } = await axiosInstance.put(
        `/seller/refurbished-orders/${orderId}/status`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (data.success) {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (row) => (
        <button
          onClick={() => router.push(`/seller/refurbished/orders/${row._id}`)}
