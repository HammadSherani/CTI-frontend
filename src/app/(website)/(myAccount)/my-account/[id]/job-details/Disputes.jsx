...
const DISPUTE_SCHEMA = yup.object({
    category: yup.string().required('Please select a dispute category'),
    description: yup
        .string()
...
const Disputes = ({ bookingId, status, dispute, fetchJob }) => {
    ...
    // Since responses have userId as plain string ID (not populated),
    // match against the known parties in the dispute object to get name
    const resolveResponder = (respondedBy) => {
        const userId = typeof respondedBy?.userId === 'object'
            ? respondedBy.userId?._id
            : respondedBy?.userId;
        const userType = respondedBy?.userType;

        if (
            dispute?.raisedBy?.userId?._id === userId ||
            dispute?.raisedBy?.userId === userId
        ) {
            return {
                name: dispute.raisedBy?.userId?.name || userType || 'User',
                userType: dispute.raisedBy?.userType || userType,
            };
        }
        if (
            dispute?.againstUser?.userId?._id === userId ||
            dispute?.againstUser?.userId === userId
        ) {
            return {
                name: dispute.againstUser?.userId?.name || userType || 'User',
                userType: dispute.againstUser?.userType || userType,
            };
        }
        return { name: userType || 'User', userType: userType || 'user' };
    };

    const getUserColor = (userType) => {
        if (userType === 'repairman') return {
            bg: 'bg-violet-100', text: 'text-violet-700',
            bubble: 'bg-violet-50 border-violet-200', name: 'text-violet-900',
            badge: 'bg-violet-100 text-violet-700',
        };
        if (userType === 'customer') return {
            bg: 'bg-blue-100', text: 'text-blue-700',
            bubble: 'bg-blue-50 border-blue-200', name: 'text-blue-900',
            badge: 'bg-blue-100 text-blue-700',
        };
        return {
            bg: 'bg-gray-100', text: 'text-gray-700',
            bubble: 'bg-gray-50 border-gray-200', name: 'text-gray-900',
    ...
    if (status === 'disputed') {
        return (
            <div className="bg-gray-50 py-3 px-4 min-h-screen flex flex-col gap-4">
                ...
                {/* Chat Card */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col" style={{ minHeight: '520px' }}>

                    {/* Chat Header */}
                    <div className="bg-gradient-to-r from-primary-50 to-primary-100 border-b border-primary-200 px-5 py-4 flex-shrink-0">
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 max-w-[85%]">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-xs px-1.5 py-0.5 bg-orange-100 text-orange-700 rounded font-medium capitalize">
                                ...
                                <div className="bg-orange-50 border border-orange-200 rounded-2xl rounded-tl-sm px-4 py-3">
                                    <span className="inline-block mb-2 px-2 py-0.5 bg-orange-200 text-orange-800 text-xs font-semibold rounded">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div>
                        ...
                        <select
                            id="category"
                            {...register('category')}
                            className={`${commonInputClasses} ${errors.category ? 'border-red-500' : 'border-gray-300'}`}
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className={`${commonInputClasses} resize-none ${errors.description ? 'border-red-500' : 'border-gray-300'}`}
                            disabled={isSubmitting}
                        />
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div className="flex gap-3">
                            <Icon icon="heroicons:exclamation-triangle" className="w-5 h-5 text-yellow-500 flex-shrink-0 mt-0.5" />
                            <div>
                    ...
                    <div className="flex gap-4">
                        <button
                            type="submit"
                            disabled={isSubmitting}
                            className={`flex-1 py-3 px-4 rounded-lg text-white font-medium flex items-center justify-center gap-2 ${isSubmitting ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary-600 hover:bg-primary-700'}`}
                        >
                            {isSubmitting && <Icon icon="heroicons:arrow-path" className="w-5 h-5 animate-spin" />}
