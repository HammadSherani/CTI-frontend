...
function Header() {
  ...
  const primaryNavLinks = [
    ...
    { name: "Ads", path: "/repair-man/ads", icon: "mdi:advertisements" },
  ];
  ...
  const dropdownLinks = [
    ...
    { name: "Earnings", path: "/earning", icon: "mdi:gavel" },
        ...
        <nav className="hidden lg:flex items-center gap-1 flex-1 justify-center">
          {primaryNavLinks.map((link) => (
            <Link
              key={link.name}
              href={link.path}
              className={`relative px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 group whitespace-nowrap
