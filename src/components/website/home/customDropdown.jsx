...
export const CustomDropdown = ({ icon, label, placeholder, options, value, onChange }) => {
  ...
  return (
    <div ref={ref} className="relative flex-1 min-w-[150px]">
      ...
      >
        {/* Left Icon */}
        <div className={`flex-shrink-0 transition-colors ${isSelected || open ? "text-primary-500" : "text-primary-600"}`}>
        ...
        <div className="flex flex-col items-start flex-1 min-w-0">
          ...
          <span className={`text-sm font-medium truncate leading-tight ${isSelected ? "text-gray-800" : "text-gray-800"}`}>
            {selectedLabel}
          </span>
        </div>
        ...
        <Icon
          ...
          className={`w-4 h-4 flex-shrink-0 text-gray-800 transition-transform duration-200 ${open ? "rotate-180 text-primary-500" : ""}`}
        />
      </button>
          ...
          <div className="max-h-52 overflow-y-auto">
            {options.map((opt) => (
              ...
              >
                {opt.label}
                {value === opt.value && (
                  <Icon icon="mdi:check" className="w-4 h-4 text-primary-500 flex-shrink-0" />
                )}
              </button>
            ))}
          </div>
